...
- `npm run db:setup` Configura/actualiza el esquema y catálogos project-residency_hotel_club
<!-- Badge de estado del workflow (reemplaza OWNER/REPO por el repo real) -->
...
Ejecutar los comandos en la terminal para instalar las dependencias del proyecto en el directorio raíz del proyecto:

npm install        # Instala todas las dependencias del proyecto
npm run dev        # Corre el entorno de desarrollo del servidor
npm run build      # Compila los estilos de TailwindCSS


Asegúrate de que la base de datos MySQL esté corriendo y que las credenciales de conexión estén correctamente configuradas en el archivo .env.
También puedes configurar la conexión vía variables de entorno (ver src/config/configuration.js).
...
Asegurarse de que el servidor de MySQL esté corriendo.

Archivo .env

Crea el archivo .env al mismo nivel del archivo de ejemplo .env.example y agrega las variables para la conexión DB y los servicios como se muestra en .env.example.

Notas adicionales

Si no creas/configuras la base de datos, el proyecto no podrá iniciar. Usa los scripts de BD provistos arriba.

Los catálogos principales (por ejemplo, metodos_pago, tipos_membresia, habitaciones) se insertan con db:setup o db:reseed de forma idempotente.

Ejecuta npm run db:health para verificar que la base de datos esté íntegra (tablas, índices, FKs, engine/collation).
En CI puedes usar HEALTH_STRICT=true. (no destruye).
- `npm run db:reseed` Reaplica INSERTs idempotentes de catálogos (no duplica).
- `npm run db:reset` DROP + CREATE + inserts (solo desarrollo).
- `npm run db:drop` Drop ultra-seguro (requiere variables de confirmación).
- `npm run db:health` Verificación de salud. En CI usar `HEALTH_STRICT=true`.
